defmodule SaltWeb.RegistrationController do
  ...
  def new(conn, _params) do
    ...
    registration = Salt.Registration.changeset(%Registration{}, map)
  ...
  def create(conn, %{"registration" => registration_params}) do
    IO.puts("REGISTRATION CREATE")

    student = Map.get(registration_params, "student_id")

    case Salt.create_registration(registration_params) do
      {:ok, registration} ->
        conn
  ...
  def index(conn, %{"student_id" => student_id} = params) do
    ...
    ################################################################
    # regs is a list of changesets - one for each class registered #
    # classes is a list of class ids-  One list of lists           #
    ################################################################
    IO.puts("CLASSES")

    classes =
      Enum.map(regs, fn x -> Salt.list_class_data(x.id, x.class_id, x.semester) end)
      |> IO.inspect()

    classtitle = params["classtitle"]

    classtitlelist = Salt.get_classtitles()
  ...
  def show(conn, %{"id" => id}) do
    ...
    |> assign(:fee, fee)
    # , layout: {SaltWeb.LayoutView, "showstudent.html"})
    |> render("show.html")

    # render(conn, "show.html", %{student: student, student_id: student_id, lastname: lastname,
    #  layout: {SaltWeb.LayoutView, "showstudent.html"}})  # use a different layout than app
  end
